...
def parking_match(keypoints,imgCamGray,orb,bf,desTrain): ### Jucha sign recognition 
	...
	for i in keypoints:
		if i[1]-90<0:
			...
			roi=imgCamGray[0:int(i[1]+90),int(i[0]-90):int(i[0]+90)]
			#cv2.imshow('sasa',roi)

		else:
			roi=imgCamGray[int(i[1]-70):int(i[1]+70),int(i[0]-70):int(i[0]+70)]
			cv2.imshow('sasa',roi)
...
def line_trace(frame,stage,verbose): ### find line then return angular velocity
	...
	cv2.line(frame,(165,155),(475,155),(253,244,8),2)
	cv2.line(frame,(165,235),(475,235),(253,244,8),2) ### draw ROI
	cv2.line(frame,(165,155),(165,235),(253,244,8),2)
	cv2.line(frame,(475,155),(475,235),(253,244,8),2)


	gray=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY) ### Process image to make finding line easy
	ROI=gray[160:230,170:470]
	ROI=cv2.GaussianBlur(ROI,(29,29),0)
	
	thr=cv2.adaptiveThreshold(ROI,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,15,2)
	blur=cv2.medianBlur(thr,3)	
	edge=cv2.Canny(blur,180,360)
	cv2.imshow('ddsd',blur)			
